export const blogPosts = [
  {
    id: 1,
    slug: 'pure-ghee-health-benefits',
    title: '১০টি কারণ: খাঁটি গাওয়া ঘি কেন প্রতিদিন খাওয়া উচিত (বিজ্ঞানসম্মত উপকারিতা)',
    englishTitle: '10 Proven Health Benefits of Pure Gawa Ghee You Should Know',
    category: 'Nutrition & Health',
    author: 'Urmira Health Desk',
    date: '28 August 2026',
    readTime: '4 min read',
    image: '/images/ghee-1.jpg',
    excerpt: 'গাওয়া ঘি কেবল খাবারের স্বাদই বাড়ায় না, এটি রোগ প্রতিরোধ ক্ষমতা বৃদ্ধি, হজম শক্তি ও মস্তিষ্কের কার্যক্ষমতা বাড়াতে অন্যতম সেরা সুপারফুড।',
    tags: ['Pure Ghee', 'গাওয়া ঘি', 'Organic Food', 'Health Tips', 'Ayurveda'],
    content: [
      {
        heading: '১. অন্ত্রের স্বাস্থ্য ও দ্রুত হজমে সহায়ক',
        body: 'খাঁটি গাওয়া ঘিয়ে রয়েছে প্রচুর পরিমাণে বিউটিরিক অ্যাসিড (Butyric Acid), যা অন্ত্রের প্রদাহ কমায় এবং খাদ্য দ্রুত পরিপাক করতে এনজাইম নিঃসরণে সাহায্য করে।'
      },
      {
        heading: '২. দীর্ঘস্থায়ী শক্তি ও স্ট্যামিনা বৃদ্ধি',
        body: 'ঘিয়ে থাকা মিডিয়াম চেইন ফ্যাটি অ্যাসিড সরাসরি লিভারে গিয়ে শক্তিতে রূপান্তরিত হয়, যা সারাদিনের কর্মব্যস্ততায় ক্লান্তি দূর করতে অনন্য।'
      },
      {
        heading: '৩. রোগ প্রতিরোধ ক্ষমতা ও হার্ট ফ্রেন্ডলি ফ্যাট',
        body: 'ঘিয়ে রয়েছে ওমেগা-৩ ও ওমেগা-৯ ফ্যাটি অ্যাসিড এবং ভিটামিন এ, ডি, ই ও কে, যা শরীরের প্রাকৃতিক রোগ প্রতিরোধ ক্ষমতাকে বহুগুণে বাড়িয়ে তোলে।'
      },
      {
        heading: '৪. ত্বক ও চোখের উজ্জ্বলতা বৃদ্ধি',
        body: 'নিয়মিত এক চামচ খাঁটি ঘি ত্বকের স্বাভাবিক আর্দ্রতা ধরে রাখে এবং দৃষ্টিশক্তি তীক্ষ্ণ করতে সহায়ক ভূমিকা পালন করে।'
      }
    ],
    relatedProductId: 1
  },
  {
    id: 2,
    slug: 'khurjur-power-bomb-energy-benefits',
    title: 'খেজুরের পাওয়ার বোম্ব: প্রাকৃতিক পুষ্টি ও ইনস্ট্যান্ট শক্তির জাদুকরী উৎস',
    englishTitle: 'Why Khurjur Power Bomb is the Ultimate Natural Energy Booster',
    category: 'Superfoods',
    author: 'Nutrition Specialist',
    date: '25 August 2026',
    readTime: '5 min read',
    image: '/images/khejur-1.jpg',
    excerpt: 'আজওয়া ও মরিয়ম খেজুর, প্রাকৃতিক মধু, কাজুবাদাম, কাঠবাদাম ও পেস্তার সমন্বয়ে তৈরি পাওয়ার বোম্ব কেন যেকোনো কৃত্রিম সাপ্লিমেন্টের চেয়ে সেরা।',
    tags: ['Power Bomb', 'Dates & Nuts', 'Instant Energy', 'Fitness', 'খেজুর পাওয়ার বোম্ব'],
    content: [
      {
        heading: '১. শূন্য কোলেস্টেরল ও শতভাগ প্রাকৃতিক ফাইবার',
        body: 'কোনো পরিশোধিত চিনি বা প্রিজারভেটিভ ছাড়াই তৈরি এই পাওয়ার বোম্ব উচ্চ আঁশযুক্ত, যা দীর্ঘক্ষণ পেট ভরা রাখে ও ওজন নিয়ন্ত্রণে সাহায্য করে।'
      },
      {
        heading: '২. ব্রেন পাওয়ার ও স্মৃতিশক্তি বৃদ্ধি',
        body: 'কাঠবাদাম ও কাজুবাদামে থাকা ভিটামিন-ই ও ম্যাগনেসিয়াম মস্তিষ্কের নিউরন সক্রিয় রাখে এবং মনোযোগ ও স্মৃতিশক্তি বাড়াতে কাজ করে।'
      },
      {
        heading: '৩. রক্তস্বল্পতা ও ক্লান্তি দূরীকরণ',
        body: 'খেজুরে থাকা প্রাকৃতিক আয়রন ও খনিজ উপাদান রক্তে হিমোগ্লোবিনের মাত্রা বাড়ায় এবং দ্রুত শারীরিক ক্লান্তি দূর করে।'
      }
    ],
    relatedProductId: 2
  },
  {
    id: 3,
    slug: 'how-to-test-pure-ghee-at-home',
    title: 'সহজ ৪টি উপায়: ঘরে বসেই কীভাবে চিনবেন আসল খাঁটি গাওয়া ঘি',
    englishTitle: '4 Easy Kitchen Tests to Verify 100% Pure Ghee at Home',
    category: 'Food Purity',
    author: 'Quality Assurance Team',
    date: '20 August 2026',
    readTime: '3 min read',
    image: '/images/ghee-2.jpg',
    excerpt: 'বাজারে ভেজাল ঘিয়ের ছড়াছড়ির মধ্যে আসল দানাদার গাওয়া ঘি শনাক্ত করার ঘরোয়া ও বৈজ্ঞানিক সহজ কৌশলগুলো জেনে নিন।',
    tags: ['Purity Test', 'Real Ghee', 'খাঁটি ঘি চেনার উপায়', 'Consumer Awareness', 'Food Safety'],
    content: [
      {
        heading: '১. হাতের তালুতে মেল্টিং টেস্ট (Melting Test)',
        body: 'এক ফোঁটা ঘি হাতের তালুতে রাখলে যদি শরীরের স্বাভাবিক তাপমাত্রাতেই ৫-১০ সেকেন্ডে গলে যায়, তবে তা শতভাগ খাঁটি ঘি।'
      },
      {
        heading: '২. তাওয়া হিট টেস্ট (Heat Test)',
        body: 'হালকা গরম পাত্রে এক চামচ ঘি দিলে যদি দ্রুত গলে গাঢ় বাদামি বর্ণ ধারণ করে এবং সুবাস ছড়ায়, তা খাঁটি হওয়ার স্পষ্ট লক্ষণ।'
      },
      {
        heading: '৩. আয়োডিন টেস্ট (Starch/Chemical Check)',
        body: 'সামান্য গলানো ঘিয়ে দুই ফোঁটা আয়োডিন যোগ করলে যদি কোনো নীল বা বেগুনি রঙ না আসে, তবে নিশ্চিত থাকুন এতে কোনো ডালডা বা স্টার্চ নেই।'
      }
    ],
    relatedProductId: 1
  },
  {
    id: 4,
    slug: 'pure-gawa-ghee-price-buying-guide-bangladesh',
    title: 'বাংলাদেশে খাঁটি গাওয়া ঘি চেনার গাইড ও ন্যায্য মূল্য নির্ধারণের উপায়',
    englishTitle: 'Pure Gawa Ghee Buying Guide & Best Price in Bangladesh',
    category: 'Nutrition & Health',
    author: 'Urmira Food Lab',
    date: '15 August 2026',
    readTime: '4 min read',
    image: '/images/ghee-3.jpg',
    excerpt: 'খামারিদের প্রস্তুতকৃত আসল দানাদার গাওয়া ঘি কেন অন্যান্য প্যাকেটজাত ঘি থেকে আলাদা এবং কেন খাঁটি ঘিয়ের দাম ন্যায্য হওয়া উচিত জেনে নিন।',
    tags: ['Gawa Ghee Price', 'গাওয়া ঘি এর দাম', 'Pure Ghee Dhaka', 'Organic Food'],
    content: [
      {
        heading: '১. কেন খাঁটি ঘিয়ের উৎপাদন খরচ বেশি?',
        body: '১ কেজি খাঁটি গাওয়া ঘি তৈরি করতে প্রায় ২৫ থেকে ৩০ লিটার খাঁটি গরুর দুধের প্রয়োজন হয়। তাই অতি কম দামে কোনো ঘি পাওয়া গেলে তাতে ভেজাল বা ডালডার মিশ্রণ থাকার আশঙ্কা প্রবল।'
      },
      {
        heading: '২. দানাদার টেক্সচার ও প্রাকৃতিক সুবাস',
        body: 'খাঁটি গাওয়া ঘি স্বাভাবিক তাপমাত্রায় সুন্দর দানাদার রূপ ধারণ করে এবং এতে হালকা বাদামি সুবাস থাকে, যা খাবারে দিলে অতুলনীয় স্বাদ বয়ে আনে।'
      },
      {
        heading: '৩. বিশ্বস্ত উৎস থেকে কেনা',
        body: 'সরাসরি খামারিদের দুধ থেকে প্রস্তুতকারী এবং ক্যাশ অন ডেলিভারিতে চেক করে কেনার সুযোগ দেয় এমন নির্ভরযোগ্য ব্র্যান্ড নির্বাচন করা বুদ্ধিমানের কাজ।'
      }
    ],
    relatedProductId: 1
  }
];
